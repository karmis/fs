...
namespace BS\WhoisBundle\Service;
...
class Whois
{
    ...
    public function init($domain)
    {
        ...
        // if(preg_match('/^[a-zа-я0-9]+([\-\.]{1}[a-zа-я0-9]+)*\.[a-zа-я\-]{2,5}(:[0-9]{1,5})?$/i', $this->domain)){
            if (
                ...
                if(isset($this->servers[$this->TLDs])) {
                    return true;
                }

                $this->error = 'tld_not_found';

                return false;
            } else {
                $this->error = 'domain_not_valid';
